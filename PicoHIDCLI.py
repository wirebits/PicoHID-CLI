...
class PicoHIDKeyboard:
    ...
    def convert_to_pico_script(pico_mnemonic):
        ...
        elif any(key in pico_mnemonic for key in hidKeys.keys()):
            keys = pico_mnemonic.split()
            key_sequence = [hidKeys[key] for key in keys if key in hidKeys]
...
def main():
    ...
    parser.add_argument('-p', '--saveas', type=str, help="Save the converted Pico script with a custom name, adding .py at the end")
